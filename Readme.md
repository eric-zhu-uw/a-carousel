...
| ❌ | arrowStyleBackward | `string` | `default` | default, \<classname\> | Applies the default styling or a custom classname for styling to the backward arrow.\ In the custom classname, utilize CSS properties: `top, bottom, left, right` for position of arrow.<br/> Utilize CSS properties: `width, height, background-color, background-image, border etc.` for styling of arrow. |
